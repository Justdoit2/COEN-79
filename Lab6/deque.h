...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        delete [] block_pointers; //original block_pointers have been deleted
        block_pointers = NULL;
       ...
       block_pointers=new value_type*[bp_array_size]; //my implementation
        //set block_pointers and block_pointers_end
        
       block_pointers_end=((block_pointers)+(bp_array_size-1)); //set block_pointers
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                if(*(source.first_bp)==source.block_pointers[bp_array_index]){
                    first_bp=block_pointers+ bp_array_index; //set first_bp to block_pointer arrayindex
                }
                
                //If this is the back_ptr of source, then set the last_bp of this deque
                // STUDENT WORK...
                if(*(source.last_bp)==source.block_pointers[bp_array_index]){
                    last_bp=block_pointers+ bp_array_index;//set last_bp to block_pointer array index
                }
                
                // Create a data block
               
                block_pointers[bp_array_index]=new int [block_size];//
        ...
        for (size_type block_item_index = 0; block_item_index < block_size;++block_item_index)
                {
                    // STUDENT WORK...
                    //
   *(block_pointers[bp_array_index]+block_item_index)=*(source.block_pointers[bp_array_index]+block_item_index);//copy
                    if((source.block_pointers[bp_array_index]+block_item_index)==source.front_ptr)
                    
                    {front_ptr=block_pointers[bp_array_index]+block_item_index;}//set
                    
                    if((source.block_pointers[block_item_index]+block_item_index)==source.back_ptr)
                    {back_ptr=block_pointers[bp_array_index]+block_item_index;}//set
    ...
    void deque<Item>::clear () {
        ...
        for (size_type index=0;index<bp_array_size;++index)
        {
            delete[] block_pointers[index];//clear data blocks
            block_pointers[index]=NULL;
        }
    ...
    void deque<Item>::push_front(const value_type& entry) //check
    {
        ...
        if (first_bp == NULL)
        {
            ...
            front_ptr=new int [BLOCK_SIZE];
            back_ptr=front_ptr;
            *(front_ptr)=entry;//assign value //bad access means nothing there
        ...
        {
            // STUDENT WORK...
            first_bp--; //go up one in array of block pointers
        ...
        {
            // STUDENT WORK...
              //need to increase size of deque, so call the reserve function
            reserve();
            first_bp--;
            front_ptr=(*first_bp)+(BLOCK_SIZE-1);
            *(front_ptr)=entry; //assign entry
    ...
    void deque<Item>::push_back(const value_type& entry)
    {
        ...
        if (last_bp == NULL)
        {
            ...
            back_ptr=new int [BLOCK_SIZE]; //allocate space for *bp
            front_ptr=back_ptr;
            *(back_ptr)=entry;//assign entry
            //allocat space for *bp, set backand front ptr=bp
            //*(front_ptr)=entry;
        }
    ...
    void deque<Item>::pop_back()
    {
        ...
        {
            // STUDENT WORK...
            delete[] back_ptr;//delete back_ptr
            --last_bp; //go up the queue
            back_ptr=(*last_bp)+ (BLOCK_SIZE-1); //set to most recent block size
