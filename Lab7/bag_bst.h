...
#pragma mark - Toolkit Function Implementation
    ...
    // to a copy of the removed item.
    {
        ...
        {
            // STUDENT WORK
            removed=root_ptr->data(); //copy the data, check
    ...
    // Postcondition: If target was in the tree, then one copy of targe
    ...
    // function returns false).
    {
        ...
        {   // Continue looking in the left subtree
            ...
            // STUDENT WORK
            bst_remove(root_ptr->left(),target); //move left
        ...
        {   // Continue looking in the right subtree
            ...
            // STUDENT WORK
            bst_remove(root_ptr->right(),target); //move right
        }
        ...
        {   // Target was found and there is no left subtree, so we can
            ...
            // STUDENT WORK
            oldroot_ptr=root_ptr; //copy
            root_ptr=root_ptr->right();//move root_ptr to right
            delete oldroot_ptr; //delete root_ptr
            return true;
        ...
        // STUDENT WORK
             bst_remove_max(root_ptr->left(),root_ptr->data()); //replace max of left to root
    ...
    // of the target were removed.
    {
        ...
        {   // Target was found and there is no left subtree, so we can
            ...
            root_ptr=root_ptr->right(); //assign new root
            delete oldroot_ptr;
            return 1; //return 1
        }
        ...
        return 1 + bst_remove_all(root_ptr, target); //recurse and add 1
    }
...
#pragma mark - Member Function Implementation
        ...
        do
        {
            ...
            {   // Go left
                ...
                if(cursor->left()==NULL){
                    ...
                    cursor->left()=insert_ptr; //assign left to entry
                    done=true; //break
                }
                cursor=cursor->left();//keep going
            ...
            {   // Go right
                ...
                if(cursor->right()==NULL){
                    cursor->right()=insert_ptr; //assign right to entry
                    done=true; //break
                }
                cursor=cursor->right(); //keep going
    ...
    typename bag<Item>::size_type bag<Item>::count(const Item& target) const
    {
        ...
        while (cursor != NULL)
        {
            ...
            else if(cursor->data()>target)
            {
                ...
                answer++; //found a target
                cursor=cursor->left();//keepy going
    ...
    // Header file used: bintree.h
    {
        ...
        tree_clear(root_ptr); //clear original
        root_ptr=tree_copy(source.root_ptr); //copy to source
    }
    ...
    {
        binary_tree_node<Item> *add_ptr;
        if (root_ptr == addend.root_ptr)
        {
            // STUDENT WORK
            add_ptr=tree_copy(addend.root_ptr); //if same root, must make a copy
            insert_all(add_ptr); //add the copy
            tree_clear(add_ptr); //clear copy
            
        }
        else
            insert_all(addend.root_ptr); //call insert_all
    }
    ...
    // method.
    {
        ...
        {
            // STUDENT WORK
            insert(addroot_ptr->data()); //insert entry
            insert_all(addroot_ptr->left());//recurse to left
            insert_all(addroot_ptr->right());//recurse to right
        }
